...
- Audit a layout-tree spec with `uv run python pptify-plugin/audit/audit.py deck-spec.json --json`.
- Run the full current test suite with `uv run python -m unittest discover -s tests -v`.
